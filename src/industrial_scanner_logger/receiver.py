...
def tracking_number_from_barcode(barcode: str, digit_count: int = TRACKING_NUMBER_LENGTH) -> str:
    cleaned = clean_barcode(barcode)

    if cleaned.isdigit() and len(cleaned) >= digit_count:
        return cleaned[-digit_count:]
...
def parse_scanner_pairs(
    scanner_pairs: str,
    field_name: str = "scanner_pairs",
) -> dict[str, tuple[str, ...]]:
    scanner_pairs = clean_barcode(scanner_pairs)

    if not scanner_pairs:
        return {}

    pair_map = {}

    for pair_text in re.split(r"\s*;\s*", scanner_pairs):
        pair_text = clean_barcode(pair_text)
        if not pair_text:
            continue

        scanner_ids = parse_configured_scanner_ids(pair_text, field_name)

        if len(scanner_ids) < 2:
            raise ValueError(
                f"{field_name} groups must include at least two scanner IDs"
            )

        scanner_group = tuple(scanner_ids)

        for scanner_id in scanner_group:
            if scanner_id in pair_map:
                raise ValueError(
                    f"{field_name} cannot include scanner {scanner_id} in more "
                    "than one group"
                )

        for scanner_id in scanner_group:
            pair_map[scanner_id] = scanner_group
...
def load_receiver_config(config_file: str = DEFAULT_CONFIG_FILE):
    ...
    try:
        ...
        return argparse.Namespace(
            ...
            scanner_pairs=parse_scanner_pairs(
            ...
            scanner_pair_suppression_distinct_successes=validate_positive_int(
                ...
                "scanners.scanner_pair_suppression_distinct_successes",
            ),
...
class PostgreSQLScanLogger:
    ...
    def _load_driver(self):
        ...
        self.insert_sql = sql.SQL(
            "INSERT INTO {}.{} "
            "(scan_timestamp, scanner_id, scanner_name, is_duplicate, is_repaired, "
    ...
    def _duplicate_progression_on_scanners(
        self,
        tracking_number: str,
        scan_timestamp: datetime,
        matching_scanner_ids: list[int],
        event_scanner_ids: list[int],
    ) -> tuple[bool, int]:
        """
        Return whether a prior match exists and its distinct-package progression.
        ...
        matching_placeholders = self._sql.SQL(", ").join(
            self._sql.Placeholder() for _scanner_id in matching_scanner_ids
        )
        event_placeholders = self._sql.SQL(", ").join(
            self._sql.Placeholder() for _scanner_id in event_scanner_ids
        )
        params = [
            tracking_number,
            lookback_start,
        ...
        query = self._sql.SQL(
            ...
            WITH last_match AS (
                ...
                WHERE is_success = true
                  ...
                  AND scanner_id IN ({matching_scanner_placeholders})
                ORDER BY scan_timestamp DESC, id DESC
                LIMIT 1
            )
            SELECT
                count(last_match.id) > 0 AS seen_before,
                count(DISTINCT events.tracking_number) AS distinct_success_count
            FROM last_match
            LEFT JOIN {table} AS events
              ON events.is_success = true
             AND events.tracking_number <> %s
             AND events.scan_timestamp BETWEEN %s AND %s
             AND events.scanner_id IN ({event_scanner_placeholders})
             AND (
            ...
            table=table_sql,
            matching_scanner_placeholders=matching_placeholders,
            event_scanner_placeholders=event_placeholders,
        )
    ...
    def _scanner_ids_for_duplicate_lookup(
        self,
        scanner_id: str,
        paired_scanner_ids: Optional[tuple[str, ...]] = None,
    ) -> list[int]:
        raw_scanner_ids = paired_scanner_ids or (scanner_id,)
        parsed_scanner_ids = []

        for raw_scanner_id in raw_scanner_ids:
            db_scanner_id = scanner_id_for_postgresql(raw_scanner_id)

            if db_scanner_id not in parsed_scanner_ids:
                parsed_scanner_ids.append(db_scanner_id)

        return parsed_scanner_ids or [scanner_id_for_postgresql(scanner_id)]
    ...
    def duplicate_state_for_success(
        ...
        db_scanner_ids = self._scanner_ids_for_duplicate_lookup(
            scanner_id,
            paired_scanner_ids,
        )
        ...
        try:
            ...
            if paired_scanner_ids_for_match:
                seen_on_paired_scanner, paired_distinct_success_count = (
                    self._duplicate_progression_on_scanners(
                        tracking_number,
                        scan_timestamp,
                        paired_scanner_ids_for_match,
    ...
    def successful_barcodes_for_repair(
        self,
        scan_date: str,
        success_length: int,
    ) -> set[str]:
...
class DailyCsvLogger:
    def __init__(
        ...
        if isinstance(scanner_pairs, str):
            self.scanner_pairs = parse_scanner_pairs(scanner_pairs)
        else:
            self.scanner_pairs = {
                validate_configured_scanner_id(
                    scanner_id,
                    "scanner_pairs keys",
                ): tuple(
                    validate_configured_scanner_id(pair_id, "scanner_pairs values")
                    for pair_id in paired_scanner_ids
                )
        ...
        self.success_barcodes_by_scanner = {}
        self.success_history_by_scanner = {}
        self.success_event_history_by_scanner = {}
    ...
    def _get_success_barcodes(self, scanner_id: str):
        duplicate_group_key = self._duplicate_group_key(scanner_id)
    ...
    def _get_success_event_history(self, scanner_id: str):
        duplicate_group_key = self._duplicate_group_key(scanner_id)
        return self.success_event_history_by_scanner.setdefault(
            duplicate_group_key,
            [],
        )
    ...
    def _paired_scanner_ids(self, scanner_id: str) -> tuple[str, ...]:
        scanner_id = normalize_scanner_id(scanner_id)
        return self.scanner_pairs.get(scanner_id, (scanner_id,))
    ...
    def _duplicate_state_for_success(
        self,
        scanner_id: str,
        tracking_number: str,
        ...
        if self.postgresql_logger is not None and scan_timestamp is not None:
            ...
            if duplicate_lookup is not None:
                ...
                if database_state is not None:
                    return (
                        bool(database_state[0]),
                        bool(database_state[1]),
    ...
    def _paired_overlap_is_within_window(
        ...
        for index in range(len(history) - 1, -1, -1):
            prior_scanner_id, prior_tracking_number = history[index]

            if (
                prior_tracking_number != tracking_number
                or prior_scanner_id == normalized_scanner_id
            ):
                continue

            intervening_tracking_numbers = {
                seen_tracking_number
                for _seen_scanner_id, seen_tracking_number in history[index + 1:]
                if seen_tracking_number != tracking_number
            }
            return (
                len(intervening_tracking_numbers)
    ...
    def _record_success_tracking_number(
        ...
        if self._classify_scan(barcode) == "SUCCESS":
            self._get_success_barcodes(scanner_id).add(barcode)
        ...
        self._get_success_event_history(scanner_id).append(
            (normalize_scanner_id(scanner_id), tracking_number)
        )

        self.success_last_index[tracking_number] = len(self.success_history)
        self.success_history.append(tracking_number)

    def _successful_barcodes_seen_today(self) -> set[str]:
        barcodes = set()

        for seen_barcodes in self.success_barcodes_by_scanner.values():
            barcodes.update(seen_barcodes)

        if self.postgresql_logger is not None and self.current_date:
            lookup = getattr(
                self.postgresql_logger,
                "successful_barcodes_for_repair",
                None,
            )

            if lookup is not None:
                try:
                    barcodes.update(lookup(self.current_date, self.success_length))
                except Exception as exc:
                    SCRIPT_LOGGER.warning(
                        "PostgreSQL repair candidate lookup failed: %s",
                        exc,
                    )
    ...
    def _repair_tracking_number(self, barcode: str) -> Optional[str]:
        ...
        for seen_barcode in self._successful_barcodes_seen_today():
            if len(seen_barcode) != self.success_length or not seen_barcode.isdigit():
                continue

            expected_overlap = seen_barcode[
            ...
            candidate = seen_barcode[:missing_prefix_length] + barcode
    ...
    def _ensure_daily_csv_header(self, csv_path: Path):
        ...
        If an older daily file exists with:
            ...
            the previous rich daily scan header

        it is migrated to the current DAILY_CSV_HEADER.
        """
    ...
    def _load_existing_day_state(self, csv_path: Path):
        ...
        success_barcodes_by_scanner = {}
        success_history_by_scanner = {}
        success_event_history_by_scanner = {}
        success_history = []
        counts_by_scanner = {}
        ...
        with csv_path.open("r", newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if status == "SUCCESS":
                    success_barcode = (
                        tracking
                        if self._classify_scan(tracking) == "SUCCESS"
                        else ""
                    )
                    tracking = tracking_number_from_barcode(tracking)
                    duplicate_group_key = self._duplicate_group_key(scanner_id)
                    ...
                    if tracking:
                        ...
                        success_event_history_by_scanner.setdefault(
                            duplicate_group_key,
                            [],
                        ).append((scanner_id, tracking))
                        success_history.append(tracking)
                    if success_barcode:
                        success_barcodes_by_scanner.setdefault(
                            duplicate_group_key,
                            set(),
                        ).add(success_barcode)
                else:
    ...
    def write_scan_event(self, raw_barcode: str, scanner_id: str = UNKNOWN_SCANNER_ID):
        scanner_id = normalize_scanner_id(scanner_id)
        captured_barcode = self._normalize_barcode_for_storage(clean_barcode(raw_barcode))
        barcode = captured_barcode
        tracking_number = tracking_number_from_barcode(captured_barcode)
        postgresql_event = None

        if not captured_barcode:
            return

        with self.lock:
            self._rotate_if_needed()

            status = self._classify_scan(captured_barcode)
            raw_status = status
            ...
            if status != "SUCCESS":
                repaired_barcode = self._repair_tracking_number(captured_barcode)

                if repaired_barcode is not None:
                    barcode = repaired_barcode
                    tracking_number = tracking_number_from_barcode(repaired_barcode)
                    ...
                    if is_repaired:
                        SCRIPT_LOGGER.info(
                            ...
                            scanner_id,
                            truncate_for_log(captured_barcode),
                            truncate_for_log(barcode),
                        )

            if status == "SUCCESS":
                if success_duplicate_state is None:
                    success_duplicate_state = self._duplicate_state_for_success(
                        scanner_id,
                        tracking_number,
                        scan_timestamp,
                ...
                seen_before, is_duplicate, suppress_paired_overlap = (
                    success_duplicate_state
                )
                if suppress_paired_overlap or (seen_before and not is_duplicate):
                    return
            ...
            self._append_scan_data_log_line(
                ...
                f"Time:{time_str} "
                f"Barcode:{truncate_for_log(captured_barcode)}"
            )
            ...
            postgresql_event = (
                ...
                not (status == "FAILED" and not captured_barcode.isdigit()),
            )
...
def scanner_log_metadata(logger: DailyCsvLogger, scanner_id: str) -> str:
    return logger._scanner_name(scanner_id) or "unmapped"
...
def handle_client(
    ...
    scanner_name = scanner_log_metadata(logger, scanner_id)
    SCRIPT_LOGGER.info(
        "Scanner connected address=%s scanner_id=%s scanner_name=%s",
        address_label(addr),
        scanner_id,
        scanner_name,
    )
